...
from aiospotify.resources.abstract.gettable_resource import GettableResource

T = TypeVar("T", bound=SpotifyObject)


class ListableResource(GettableResource, Generic[T]):
    ...
    async def get_all(self, *, offset: int = 0) -> List[T]:
        ...
        pages = await asyncio.gather(*[self.get(offset=o) for o in offsets])
        for page in pages:
