...
```bash
STACK_NAME=next-api-smoothify-tasercake-com sh ./scripts/deploy.sh
```
...
services:
  db:
    ...
    deploy:
      placement:
        constraints:
          - node.labels.api-smoothify-tasercake-com.app-db-data == true
```
...
```bash
docker node update --label-add api-smoothify-tasercake-com.app-db-data=true dog.example.com
```

* Then you need to do the same for each stack version you have. For example, for staging you could do:

```bash
docker node update --label-add next-api-smoothify-tasercake-com.app-db-data=true cat.example.com
```
...
```bash
# Use the environment variables passed to this script
